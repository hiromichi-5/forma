...
通知種別ごとに、その設定で実際に送信されるメールの件名と HTML 本文を表示する。`include_detail` を切り替えると表示も切り替わるため、ステータス名・担当者名が回答者に渡ることをその場で確認できる。`mode = off` の場合も「通知を有効にすると送信されるメール」として表示する。

プレビューはフロントエンドだけで組み立てる。`frontend/src/lib/notification-email-preview.ts` にテンプレートの写しを持ち、`{{...}}` を単純置換する（バックエンドの `replaceVars` と同じ方式）。差し込む値は以下。

| 変数 | プレビューで使う値 | 値がない場合 |
| --- | --- | --- |
| `form_title` | フォーム名 | — |
| `status_name` | フォームの末尾のステータス名 | `対応中` |
| `assignee_name` | 閲覧者自身の表示名| `担当者` |

本文はテンプレート由来の HTML をそのまま `iframe`（`sandbox=""`）へ描画する。アプリ側の CSS と混ざらないようにするためと、スクリプトを実行させないため。
...
### メールのプレビューをサーバーでレンダリングする

`GET /v1/forms/:form_id/notification-previews` を追加し、`EmailSender` に送信せずレンダリングだけを行う `PreviewEmail` を生やして、種別 × 粒度の全組み合わせを返す案。

- メリット: 実際に送信されるものと同じテンプレート・同じ差出人から組み立てるため、プレビューと送信内容が必ず一致する。テンプレートの二重管理も生じない
- デメリット: OpenAPI の更新と `gen.go` の再生成、ハンドラ、ユースケース（フォームとステータスの取得を伴う）、`EmailSender` インタフェースの拡張とモックの追随まで一式が必要になる。表示するだけの機能に対して実装量が見合わない

**不採用理由:** プレビューは参考表示であり、正確さより実装量の小ささを優先したため。テンプレートが二重管理になることは許容する。
...
- **メールテンプレートがフロントエンドにも複製されている**。設定画面のプレビュー用の写しであり、`infra/resend/templates/` を変更したときは `frontend/src/lib/notification-email-preview.ts` もあわせて更新しないと、プレビューと実際の送信内容が食い違う
- **テンプレートへの変数の差し込みは HTML エスケープを行っていない**。`replaceVars` は単純置換のため、フォーム名やステータス名に `<` などを含めるとメール本文の HTML が壊れる。プレビュー側は表示崩れを避けるためエスケープしており、この点だけ実際の送信内容と異なる
